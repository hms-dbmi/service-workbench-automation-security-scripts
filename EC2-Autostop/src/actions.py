    ...
    if activation_tag == 'not_found':
        ...
        for tag in default_alarms['AWS/Lambda']:
            ...
            AlarmName = alarm_separator.join(
                [alarm_identifier, function_name, Namespace, MetricName, ComparisonOperator,
                 Period, Statistic])
...
def create_alarm_from_tag(instance_id, alarm_tag, instance_info, metric_dimensions_map, sns_topic_arn, alarm_separator,
    ...
    create_alarm(AlarmName, MetricName, ComparisonOperator, Period, alarm_tag['Value'], Statistic, namespace,
                 dimensions, sns_topic_arn , threshold)
...
def process_alarm_tags(instance_id, instance_info, default_alarms, metric_dimensions_map, sns_topic_arn, cw_namespace,
    ...
    logger.info('Instance type is: {}'.format(instance_type))
