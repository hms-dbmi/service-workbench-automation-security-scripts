...
def lambda_handler(event, context):
    try:
        all_instance_ids = get_instance_ids("swb-aim-ahead-unused-instances-artifacts", "instances-launched-21daysago.csv")
        logged_instance_ids = get_instance_ids("aimahead-swb-prod-notification-artifacts", "swb_day21_loggedIn_instances.csv")
        instances_not_logged = set_difference(all_instance_ids, logged_instance_ids)
        write_to_bucket("swb-aim-ahead-unused-instances-artifacts", "swb-day21-instances-not-loggedIn.csv", instances_not_logged)
